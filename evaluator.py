...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} AUC.\n'
            .format(self.dataset, self.method, mae, max_f, mean_f, auc))
        return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} AUC.'.format(
            time.time() - start_time, self.dataset, self.method, mae, max_f, mean_f, auc)
